...
(async()=>{
 ...
 const jumpCover=await page.evaluate(()=>{const o=world.obstacles[0];player.x=o.x-player.w/2-14;player.z=o.z;player.lift=0;player.jumpV=0;player.ground=true;player.dash=0;player.vx=0;player.vz=0;pressed.add('Space');keys.add('KeyD');for(let i=0;i<20;i++)update();keys.clear();for(let i=0;i<30;i++)update();return player.ground&&Math.abs(player.lift-coverTopAt(o,player.x+player.w/2,player.z))<.001&&player.x+player.w/2>o.x});assert(jumpCover);
 await reset();
 ...
 const newMapRules=await page.evaluate(()=>{
 for(let s=0;s<6;s++){stage=s;loadStage();if(world.sectors.length<4||new Set(world.obstacles.map(o=>o.type)).size<4)return 'missing map variation';player.x=80;player.z=130;refreshFlow();
  for(const item of pickups){if(!canStand(item.x+12,item.z,10,8,0))return 'supply inside cover';let reachable=false;for(let dx=-20;dx<=20;dx+=10)for(let dz=-20;dz<=20;dz+=10){const cell=navCell(item.x+12+dx,item.z+dz);if(world.nav.grid[cell]&&world.nav.dist[cell]>=0)reachable=true}if(!reachable)return 'unreachable supply'}
  for(let n=0;n<200;n++){const x=(n*131.7)%stages[s].length,z=MAP_BACK+(n*17.3)%(MAP_FRONT-MAP_BACK),lift=n%3*25;const brute=!world.obstacles.some(o=>lift<coverTopAt(o,x,z)-.05&&footprintOverlap(o,x,z,12,10));if(canStand(x,z,12,10,lift)!==brute)return 'spatial index mismatch'}
 }
 stage=0;loadStage();const tank=world.obstacles.find(o=>o.round),barrier=world.obstacles.find(o=>o.type==='barrier');
 if(!tank||!barrier)return 'missing shapes';if(canStand(tank.x+tank.w/2,tank.z,0,0,0))return 'tank center not solid';if(!canStand(tank.x+1,tank.z-tank.d/2+1,0,0,0))return 'tank corner falsely blocked';
 const hit=coverImpact({x:barrier.x+barrier.w+90,z:barrier.z,alt:8},{x:barrier.x-90,z:barrier.z,alt:8});if(!hit||hit.cover!==barrier||hit.t<=0||hit.t>=1)return 'swept shot tunneled';
 const from={x:tank.x-20,z:tank.z-tank.d/2+1,alt:10},to={x:tank.x+5,z:from.z,alt:10};if(coverImpact(to,from))return 'round corner swept false positive';
 const above=100;if(coverImpact({x:barrier.x+barrier.w+90,z:barrier.z,alt:above},{x:barrier.x-90,z:barrier.z,alt:above}))return 'high shot blocked';
 const c=world.obstacles.find(o=>o.type==='container');player.x=c.x+10;player.z=c.z-c.d/2-16;player.lift=0;player.supportCover=null;placeActor(player);if(!coverOccludesPlayer(c))return 'occlusion fade missing';player.z=c.z+c.d/2+20;placeActor(player);if(coverOccludesPlayer(c))return 'front player falsely occluded';
 return true;
 });assert.equal(newMapRules,true);
 ...
 assert.deepEqual(errors,[]);console.log('PASS: normalized WASD/diagonal movement, continuous depth bounds, jump+movement, terrain, enemy pursuit/strafe, directional shooting, spatial collisions, enemy aim, radial grenade/flight, ammo/reload/pause, supply caps, grenade limits, immunity, six-stage campaign, restart, cache/particle bounds, solid cover, jump-on-cover, dash collision/cooldown, buffered jumps, enemy cover navigation, six connected maps, sprite/effect budgets, quality toggle, six themed sector maps, reachable supplies, spatial index equivalence, cylindrical footprints, swept projectile collision, occlusion fade and mobile controls.');
