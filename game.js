...
function shoot(){
 ...
 tone(powered?260:180,.045);burst(player.x+13+player.aimX*23,player.y+20+player.aimZ*23,'#ffe394',2);
}
...
function explode(g){burst(g.x,g.y??groundAt(g.x,depthOf(g)),'#ffb24e',45,depthOf(g));shake=12;tone(65,.3,'sawtooth',.08);for(const e of [...enemies,boss])if(Math.hypot(e.x+e.w/2-g.x,depthOf(e)-depthOf(g))<145)damage(e,12);bullets=bullets.filter(b=>b.owner==='player'||Math.hypot(b.x-g.x,depthOf(b)-depthOf(g))>160)}
...
function update(){
 ...
 let mx=Number(down('ArrowRight','KeyD'))-Number(down('ArrowLeft','KeyA')),mz=Number(down('ArrowDown','KeyS'))-Number(down('ArrowUp','KeyW'));const length=Math.hypot(mx,mz)||1;
 p.vx=mx/length*3.7;p.vz=mz/length*3.7;if(mx||mz){p.aimX=mx/length;p.aimZ=mz/length;if(mx)p.face=Math.sign(mx)}
 if((pressed.has('Space')||pressed.has('KeyL'))&&p.ground){p.jumpV=10.5;p.ground=false;tone(330,.08,'triangle')}
 ...
 for(const b of bullets){b.x+=b.vx;b.z+=b.vz;b.alt+=b.va;b.y=groundAt(b.x,b.z)-b.alt;b.life--;if(b.owner==='player'){for(const e of [...enemies,boss]){if(e.hp>0&&(e!==boss||boss.active)&&projectileHits(b,e)){damage(e,1);b.life=0;break}}}else if(projectileHits(b,p)){hurt();b.life=0}}
...
function soldier(e,friendly){
 const x=Math.round(e.x),y=Math.round(e.y),face=friendly?pFace():Math.sign(player.x-e.x)||-1;let walk=friendly?Math.hypot(player.vx,player.vz)>0:true;let leg=walk?Math.sin(tick*.22)*5:0;
 ...
 const ax=(e.aimX??face)*face,az=e.aimZ||0;ctx.save();ctx.translate(5,20);ctx.rotate(Math.atan2(az,ax));rect(0,-3,30,6,'#253738');rect(25,-3,9,3,'#bac6b2');rect(4,-5,17,2,'#879a88');rect(7,3,5,7,'#182728');ctx.restore();
